					...
					<div className="ts-card max-w-lg w-full">
						...
						<div className="p-4 space-y-3 text-sm">
							<div className="text-[var(--ts-text-muted-70)]">
								AI Summary: 8+ years building scalable React/Node systems;
								strong in TypeScript and AWS.
							</div>
							<div className="text-[var(--ts-text-muted-70)]">
								Key Skills: React, TS, Node, AWS
							</div>
							<div
								className="border rounded p-3"
								style={{ borderColor: "var(--ts-border)" }}
							>
								<div className="font-medium text-[var(--ts-text)] mb-1">
									Structured Feedback
								</div>
								<div className="grid grid-cols-3 gap-2 text-xs">
									<label className="flex items-center gap-2">
										Tech <input type="range" min={1} max={5} defaultValue={4} />
									</label>
									<label className="flex items-center gap-2">
										Team Fit{" "}
								...
								<textarea
									className="mt-2 w-full h-20 bg-transparent border rounded p-2 text-[var(--ts-text)]"
									style={{ borderColor: "var(--ts-border)" }}
									placeholder="Brief justification"
								/>
								<div className="mt-2 flex items-center gap-3 text-xs">
									<label className="flex items-center gap-1">
										<input type="radio" name="next" defaultChecked /> Recommend
										Interview
									</label>
									<label className="flex items-center gap-1">
										<input type="radio" name="next" /> Not a Fit
									</label>
									<label className="flex items-center gap-1">
										<input type="radio" name="next" /> Hold
									</label>
								</div>
							</div>
						</div>
